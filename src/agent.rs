...
impl Agent {
    ...
    pub async fn chat_once_with_attachments_metadata_runtime(
        ...
        let response = self
            .complete_turn_with_tools(turn.messages, runtime)
            .await?;
        if !turn.synthetic {
    ...
    async fn complete_turn_with_tools(
        ...
        self.complete_turn_with_tools_config(messages, runtime, false, false)
            .await
    }
    ...
    for iteration in 0..MAX_TOOL_ITERATIONS {
        ...
        if record_tool_messages {
            context.memory.messages.add(
                "assistant",
                &text,
                Some(json!({ "tool_calls": tool_calls.clone() })),
            )?;
        }
        request
            .messages
...
pub fn prepare_turn(
    ...
    let recent = history_records_for_turn(
        memory.messages.get_recent(history_fetch_limit(options))?,
    ...
    let system = build_system_prompt(memory, prompts, recall.as_deref())?;
...
fn build_system_prompt(
    ...
    let mut parts = vec![instructions, memory_context];
