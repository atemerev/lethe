...
use crate::chat::{
	...
	ChatStreamResponse, ContentPart, MessageContent, PromptTokensDetails, ReasoningEffort, ToolCall, Usage,
};
...
impl AnthropicAdapter {
	...
	fn into_anthropic_request_parts(chat_req: ChatRequest) -> Result<AnthropicRequestParts> {
		...
		// (content, cache_control variant if any) — Lethe fork: carry the
		// variant so we can distinguish Ephemeral (5min) from Persistent (1h).
		...
		for msg in chat_req.messages {
			let cache_control = msg.options.as_ref().and_then(|o| o.cache_control);
			...
			match msg.role {
				...
				ChatRole::User => {
					if msg.content.is_text_only() {
						...
						let content = apply_cache_control_to_text(cache_control, text);
						...
						let values = apply_cache_control_to_parts(cache_control, values);
		...
		// Lethe fork: stamp cache_control on EVERY system part that requests it
		// (upstream stamped only the last). Anthropic accepts up to 4 cache
		// breakpoints per request, and a multi-tier prompt (stable+volatile)
		// benefits from one breakpoint per tier.
		let system = if !systems.is_empty() {
			let any_cache = systems.iter().any(|(_, control)| control.is_some());
			let system: Value = if any_cache {
				let mut parts: Vec<Value> = Vec::new();
				for (content, control) in systems.iter() {
					let mut part = json!({"type": "text", "text": content});
					if let Some(control) = control
						&& let Some(obj) = part.as_object_mut()
					{
						obj.insert("cache_control".to_string(), cache_control_value(*control));
					}
					parts.push(part);
				}
				json!(parts)
...
/// Lethe fork: build the Anthropic cache_control object for a CacheControl
/// variant. `Ephemeral` is the upstream default (5min TTL implicit);
/// `Persistent` stamps the 1-hour extended TTL.
...
fn apply_cache_control_to_text(cache_control: Option<CacheControl>, content: String) -> Value {
	if let Some(control) = cache_control {
		let value = json!({"type": "text", "text": content, "cache_control": cache_control_value(control)});
		json!(vec![value])
	}
	// simple return
	else {
...
fn apply_cache_control_to_parts(cache_control: Option<CacheControl>, parts: Vec<Value>) -> Vec<Value> {
		...
		&& !parts.is_empty()
	{
		...
		if let Some(last_value) = parts.get_mut(len - 1) {
			...
			let _ = last_value.x_insert("cache_control", cache_control_value(control));
			// TODO: Should warn
		}
	}
	parts
